...
class ChecklistBackupEntry {
  const ChecklistBackupEntry({
    required this.id,
...
class ChecklistBackupService {
  const ChecklistBackupService({
  ...
  Future<void> saveAutomaticBackup(
    ...
    final File backupFile = File(
      '${directory.path}/${_timestampedFileName()}',
    );
    await backupFile.writeAsString(exportJson, flush: true);
    await _pruneExcessBackups(<File>[backupFile, ...files]);
  }

  Future<List<ChecklistBackupEntry>> listBackups() async {
    final Directory directory = await _backupDirectory();
    if (!await directory.exists()) {
      return const <ChecklistBackupEntry>[];
    }
    ...
    final List<ChecklistBackupEntry> backups = <ChecklistBackupEntry>[];
    for (final File file in files) {
      final DateTime savedAt = await file.lastModified();
      backups.add(
        ChecklistBackupEntry(
  ...
  String _timestampedFileName() {
    ...
    return 'checklist-templates-auto-backup-$sanitizedTimestamp.json';
  }
}
